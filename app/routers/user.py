...
# маршрутизации. 1.1"
# Цель: научиться управлять записями в БД, используя SQLAlchemy и
...
# к нашему роутеру коннектим блок, который будет позволять добавлять новых пользователей:
@router.post("/create")
...
async def update_user(user_id: int, update_user: UpdateUser, db: Annotated[Session, Depends(get_db)]):
    ...
    if user:
        db.execute(update(User).where(User.id == user_id).values(**update_user.dict()))
        db.commit()
